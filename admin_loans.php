...
$query = "SELECT apply_loan_id, account_no, p_firstname, p_middlename, p_lastname, contact_no_pensioner, 
          p_street_no, p_barangay, p_municipality, p_province, customer_code, status FROM apply_loan";
                        ...
                        <?php  
                            if ($result->num_rows > 0) {
                                while($row = $result->fetch_assoc()) {
                                    ...
                                    echo '<td></td>'; // Placeholder for Loan Detail column
                                    // Conditionally render the "View" button based on loan status from the database
                                    echo '<td><div class="btns">';
                                    if ($row["status"] === "Released") {
                                        echo '<button class="edit-btn" onclick="openModal(' . $row['apply_loan_id'] . ')">View</button>';
                                    }
                                    echo '</div></td>';
                                    // Placeholder for Payment Detail
        ...
        <input type="hidden" name="apply_loan_id" id="apply_loan_id" value="">
            ...
            <div class="form-group1">
                ...
                <select id="claim_type" name="claim_type" required>
                    ...
                    <option value="sss">Personal Loan</option>
                    <option value="pvao">Emergency Loan</option>
                    <option value="gsis">Business Loan</option>
                </select>
            </div>
