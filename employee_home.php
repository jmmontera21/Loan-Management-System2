...
// Calculate the total amount of loans released
$totalLoanBorrowedQuery = "SELECT SUM(loan_amount) AS total_borrowed FROM apply_loan WHERE status = 'Released'";
$totalLoanBorrowedResult = mysqli_query($conn, $totalLoanBorrowedQuery);
$totalLoanBorrowedRow = mysqli_fetch_assoc($totalLoanBorrowedResult);
$totalLoanBorrowed = $totalLoanBorrowedRow['total_borrowed'] ?? 0; // Fallback to 0 if no results

// Calculate the total amount of funds retrieved back to the company
$totalFundsRetrievedQuery = "SELECT SUM(paid_amount) AS total_retrieved FROM loan_payments";
$totalFundsRetrievedResult = mysqli_query($conn, $totalFundsRetrievedQuery);
$totalFundsRetrievedRow = mysqli_fetch_assoc($totalFundsRetrievedResult);
$totalFundsRetrieved = $totalFundsRetrievedRow['total_retrieved'] ?? 0; // Fallback to 0 if no results
...
$loanTypes = ['SSS', 'GSIS', 'PVAO']; // Define the loan types to include in the chart
$customerCountByLoanType = array_fill_keys($loanTypes, 0); // Initialize all types to 0

$sql = "SELECT loan_type, COUNT(*) as count 
        FROM apply_loan 
        WHERE status IN ('Approved', 'Released', 'For Approval', 'Rejected') 
        AND loan_type IN ('SSS', 'GSIS', 'PVAO') 
        GROUP BY loan_type";
$result = mysqli_query($conn, $sql);
if ($result) {
    while ($row = mysqli_fetch_assoc($result)) {
...
<body>
            ...
            <div class="cards">
                ...
                <div class="cards-container">
                    <div class="card" style="background-color: #44bd32;">
                        <h3>Total Funds Borrowed</h3>
                        <h2>₱<?php echo number_format($totalLoanBorrowed, 2); ?></h2>
                    </div>
                    <div class="card" style="background-color:darkcyan;">
                        <h3>Total Funds Retrieved</h3>
                        <h2>₱<?php echo number_format($totalFundsRetrieved, 2); ?></h2>
                    </div>
                </div>
            </div>
            <div class="cards">
    ...
    // Ensure all loan types are present in the dataset
    customerCountByLoanType = {
        'SSS': customerCountByLoanType['SSS'] || 0,
        'GSIS': customerCountByLoanType['GSIS'] || 0,
        'PVAO': customerCountByLoanType['PVAO'] || 0
    };
    ...
    var loanTypeChart = new Chart(ctx2, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: [
                    'rgba(255, 99, 132, 0.6)', // SSS
                    'rgba(54, 162, 235, 0.6)', // GSIS
                    'rgba(255, 206, 86, 0.6)'  // PVAO
                ],
                borderColor: [
                    'rgba(255, 99, 132, 1)', // SSS
                    'rgba(54, 162, 235, 1)', // GSIS
                    'rgba(255, 206, 86, 1)'  // PVAO
                ],
                borderWidth: 1
            }]
        },
